...
const DefaultMenuPreview = ({ restaurant, menu, device = 'mobile' }) => {
  ...
  useEffect(() => {
    if (iframeRef.current && restaurant?.id) {
      setLoading(true);
      setError(false);
      const url = `${window.location.origin}/placeorder?restaurant=${restaurant.id}`;
...
export default DefaultMenuPreview;
