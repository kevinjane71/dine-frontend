...
const BookMenuPreview = ({ restaurant, menu, device = 'mobile' }) => {
  ...
  useEffect(() => {
    if (iframeRef.current && restaurant?.id) {
      setLoading(true);
      setError(false);
      const url = `${window.location.origin}/placeorder/book?restaurant=${restaurant.id}`;
...
export default BookMenuPreview;
