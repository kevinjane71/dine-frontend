...
const CubeMenuPreview = ({ restaurant, menu, device = 'mobile' }) => {
  ...
  useEffect(() => {
    if (iframeRef.current && restaurant?.id) {
      setLoading(true);
      setError(false);
      const url = `${window.location.origin}/placeorder/cube?restaurant=${restaurant.id}`;
...
export default CubeMenuPreview;
